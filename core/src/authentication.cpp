	...
	const boost::urls::url_view& url)
{
	...
	if (parsed_message.end() == authorization_iter) {
		...
		if (params.end() == credentials_iter) {
			logging::debug("No Authorization header or Credential parameter found.");
			return std::nullopt;
		}
		...
		if (params.end() == credentials_iter) {
			logging::debug("No SignedHeaders query parameters found.");
			return std::nullopt;
		}
		...
		if (params.end() == date_iter) {
			logging::debug("No Date query parameter found.");
			return std::nullopt;
		}
		current_timestamp = (*date_iter).value;
		...
		if (params.end() == signature_iter) {
			logging::debug("No Signature query parameter found.");
			return std::nullopt;
		}
		...
		current_timestamp = parsed_message.at("X-Amz-Date");
	}
