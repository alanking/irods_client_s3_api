	...
	const boost::urls::url_view& url)
{
	...
	try {
		if (!fs::client::exists(conn, path)) {
			logging::debug("{}: Could not find {}", __func__, path.string());
			response.result(beast::http::status::not_found);
			logging::debug("{}: returned [{}]", __func__, response.reason());
			session_ptr->send(std::move(response));
			return;
		...
		if (fs::client::is_collection(conn, path)) {
			if (fs::client::remove_all(conn, path, fs::remove_options::no_trash) >= 0) {
				logging::debug("{}: Remove {} (collection) successful", __func__, path.string());
			...
			if (fs::client::remove(conn, path, experimental::filesystem::remove_options::no_trash)) {
				logging::debug("{}: Remove {} (object) successful", __func__, path.string());
				response.result(beast::http::status::ok);
			}
			else {
				response.result(beast::http::status::forbidden);
